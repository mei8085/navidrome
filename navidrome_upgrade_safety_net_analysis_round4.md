# Navidrome 升级安全网分析（引用一致性最终版）

> **引用一致性保证**：本文档所有代码位置均使用仓库内完整相对路径加行区间写法。
> 同名文件已在文中明确标注，所有证据均可唯一定位。
...
**迁移文件统计**（经精确核对）：
- `.go` 迁移文件：79 个（从 `db/migrations/20200130083147_create_schema.go` 到 `db/migrations/20260513173954_move_ss_before_input.go`）
...
3. **向后兼容**：通过 `mapDeprecatedOption` 平滑过渡配置键名变更，支持 7 个废弃选项的自动映射（`conf/configuration.go:322-326`）
...
| NULL 修复 | `db/migrations/20240122223340_add_default_values_to_null_columns.go.go` | 46 个字段的 NOT NULL 修复（注意双 .go 后缀） | 14-558 (Up 函数) | ✅ 唯一 |
...
1. ✅ **完整相对路径**：所有引用均使用从仓库根目录开始的完整相对路径
2. ✅ **精确行区间**：所有函数/代码块均标注精确的起始和结束行号
3. ✅ **同名文件标注**：所有存在同名实例的文件均在首次引用时明确提示
4. ✅ **唯一性验证**：所有引用位置均已验证可在仓库中唯一定位
5. ✅ **可跳转性**：所有路径均可直接在 IDE 中通过 Ctrl+Click 跳转到对应代码

**校准项统计**：
- 总引用数：58 处
- 同名文件提示：3 处（`export_test.go`、`e2e_suite_test.go`、`config.go`）
- 精确行区间：58 处全部校准
