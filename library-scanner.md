...
| 场景 | 代码位置 | 行为 |
|-----|---------|------|
| 启动时 | `db/db.go:50-55` | `true` 时执行 `PRAGMA optimize=0x10002` |
| 迁移后 | `db/db.go:106-111` | `true` 且有 schema 变更时执行 `PRAGMA optimize` |
| 扫描时（ScanEnd） | `library_repository.go:182-184` | `true` 时执行 `PRAGMA optimize=0x10000` |
