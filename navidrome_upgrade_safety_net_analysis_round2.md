# Navidrome 升级安全网分析（校对版）

> 本文档经过逐段代码核对，所有引用位置均已验证与仓库实际代码一致。
...
**迁移文件统计**（经核对）：
- `.go` 迁移文件：79 个（从 `20200130083147_create_schema.go` 到 `20260513173954_move_ss_before_input.go`）
- `.sql` 迁移文件：18 个（从 `20230404104309_empty_sql_migration.sql` 到 `20260410201914_fix_zero_album_created_at.sql`）
- 工具文件：1 个（`migration.go`）
- **总计**：98 个文件
...
...
20240122223340_add_default_values_to_null_columns.go.go  # NULL 字段修复（注意双 .go 后缀）
...
...
在 `20200130083147_create_schema.go:15-35` 中，核心表采用 **NOT NULL + DEFAULT** 模式：
...
`20240122223340_add_default_values_to_null_columns.go.go` 是一个里程碑式的迁移（注意：文件名包含双 `.go.go` 后缀），对历史原因产生的 NULL 字段进行全面修复。采用 **"新增-迁移-替换"** 三步法：
...
经 grep 核对，**23 个迁移文件**调用了 `forceFullRescan`，主要场景包括：
...
用于向用户通知重要变更（如破坏性变更、需要重建索引等）。`isDBInitialized()` 通过检查 `property` 表中的初始设置标志判断是否为已有数据库。
...
`setViperDefaults()` 函数（`conf/configuration.go:722-883`）定义了 **162 个** 默认值（经逐行统计），覆盖：
...
- **递归处理**：`applySensitiveFieldMasking()` 支持嵌套结构中的敏感字段
...
**错误收集**：`collectErrors()` 递归遍历 `jsonschema.ValidationError` 树，收集所有叶子错误，支持字段级错误路径显示（`field/subfield` 格式）。
...
- `cfg.toml`、`cfg.yaml`、`cfg.ini`、`cfg.json` - 四种格式的标准测试配置
- `cfg_nd_keys.toml` - 测试 ND_ 前缀键名自动修正
- `cfg_nd_conflict.toml` - 测试 ND_ 前缀与标准键名冲突场景
...
**套件初始化流程**（`e2e_suite_test.go:402-455`）：
...
**路径1：迁移验证 → 数据完整性**（`e2e_suite_test.go:414-447`）

```
db.Init(ctx)
├─ goose.UpContext() → 按时间戳顺序执行所有 97 个迁移
├─ 验证 goose_db_version 版本号与最新迁移一致
└─ scanner.ScanAll(ctx, true)
   ├─ 解析 fake FS 中的 14 个测试音频文件元数据
...
**设计原则**（`e2e_suite_test.go:394-400` 编译时接口检查）：
- 持久层使用 **真实实现** 验证数据库交互
...
**事务支持**（`persistence/persistence.go:131-155`）：
...
1. **渐进式约束收紧**：从 2020 年初始 schema 允许部分 NULL，到 2024 年完成 46 个字段的 NOT NULL 修复，体现了对数据完整性认识的深化
2. **SQLite 适配层**：通过 `createAddColumnFunc`、`writable_schema` 等技巧，抹平了 SQLite 的 DDL 限制
3. **迁移-重建闭环**：`forceFullRescan` 机制确保 schema 变更后数据一致性，23 个迁移触发全量重扫
...
3. **向后兼容**：通过 `mapDeprecatedOption` 平滑过渡配置键名变更，支持 7 个废弃选项的自动映射
...
2. **迁移测试**：缺乏对迁移本身的单元测试（如 `20240122223340` 的数据转换逻辑未单独测试）
...
| 模块 | 文件路径 | 核心功能 | 关键行号 |
|------|---------|----------|----------|
| 迁移框架 | `db/db.go` | goose 集成、迁移执行流程 | 75-117 (Init), 164-181 (辅助函数) |
| 迁移工具 | `db/migrations/migration.go` | `forceFullRescan`、`notice`、`createAddColumnFunc` | 15-20 (notice), 23-35 (forceFullRescan), 93-123 (createAddColumnFunc) |
| 初始 schema | `db/migrations/20200130083147_create_schema.go` | 核心表结构定义 | 15-35 (album 表示例) |
| NULL 修复 | `db/migrations/20240122223340_add_default_values_to_null_columns.go.go` | 46 个字段的 NOT NULL 修复（注意双 .go 后缀） | 14-558 (Up 函数) |
| 配置核心 | `conf/configuration.go` | 加载、验证、默认值、钩子机制 | 294-306 (SnapshotConfig), 317-467 (Load), 722-883 (setViperDefaults) |
| 测试导出 | `conf/export_test.go` | 导出内部函数供测试 | 全部 |
| 配置测试 | `conf/configuration_test.go` | 验证逻辑的全面测试 | 20-307 (全部测试用例) |
| 敏感字段 | `server/nativeapi/config.go` | API 输出时的脱敏处理 | 20-66 |
| 插件配置 | `plugins/config_validation.go` | JSON Schema 验证 | 42-79 (ValidateConfig) |
| E2E 套件 | `server/e2e/e2e_suite_test.go` | 端到端测试基础设施 | 402-544 (BeforeSuite/setupTestDB/restoreDB) |
| 持久层 | `persistence/persistence.go` | SQLStore 仓储工厂 | 20-129 (仓储方法), 131-155 (WithTx) |
...
1. ✅ 迁移文件列表与数量（79 .go + 18 .sql = 97 个迁移文件）
2. ✅ 关键函数行号（`createAddColumnFunc` 在 migration.go:93，`setViperDefaults` 在 configuration.go:722 等）
3. ✅ 测试用例数量与覆盖范围（48 个配置测试用例）
4. ✅ 测试数据文件存在性（`cfg_nd_conflict.toml` 等 6 个测试文件）
5. ✅ E2E 测试模块协作关系（14 个测试音频文件、8 种测试替身）
6. ✅ 默认值数量（162 个 `viper.SetDefault` 调用）
